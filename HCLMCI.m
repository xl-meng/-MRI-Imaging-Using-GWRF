clc;clear;
load('feature_filter_MulGeneP_HC_LMCI.mat');
load('train_valid_HCLMCI.mat');
...
HC = -ones(1, 310);
LMCI = ones(1, 390);%给AD和HC设定标签数目，HC为-1，LMCI为1
labels = [LMCI, HC];%组合标签

x=all_x(2,:);
% x = randperm(700);%随机生成一个矩阵，大小为700*1，值为1-700  

N_train = floor(0.6 * 700);%取60%为训练集
N_v = floor(0.2 * 700);%取20%为验证集
N_test = 700-N_train-N_v;%取20%为测试集
train_ = x(1:N_train);%取对应的index
v_ = x(N_train+1:N_train+N_v);
test_ = x(N_train+N_v+1:700);

PRECT = length(feature_filter_MulGeneP_HC_LMCI); %特征的总数 
...
for NIND=300:20:500
    ...
    % 随机森林 & 随机森林+权重
    [Acc_RF_final,Acc_RF_Weighted_final,Acc_valid,result_valid,result_rf,result_wrf,v_lab,test_lab] = RF_use(feature,NIND,feature_filter_MulGeneP_HC_LMCI,labels,train_,v_,N_v,test_,N_test);
    %保存验证集结果
    ...
    % 随机森林+权重+遗传 & 随机森林+遗传
    [Acc_RF_GA_final,Acc_valid1,result_ga_valid,result_garf,sample_ga] = GA_RF_use(PRECT,feature,NG,NIND,feature_filter_MulGeneP_HC_LMCI,labels,train_,v_,N_v,test_,N_test);
    [Acc_RF_GA_Weighted_final,Acc_valid2,result_gawf_valid,result_gawrf,sample_gawf] = GAWF_RF_use(PRECT,feature,NG,NIND,feature_filter_MulGeneP_HC_LMCI,labels,train_,v_,N_v,test_,N_test);
    %保存验证集结果
