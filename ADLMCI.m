...
AD = -ones(1, 296);
LMCI = ones(1, 390);%给AD和HC设定标签数目，AD为-1，LMCI为1
labels = [LMCI, AD];%组合标签
x=all_x(84,:);%随机生成一个矩阵，大小为686*1，值为1-686  
N_train = floor(0.6 * 686);%取60%为训练集
N_v = floor(0.2 * 686);%取20%为验证集
N_test = 686-N_train-N_v;%取20%为测试集

train_ = x(1:N_train);%取对应的index
v_ = x(N_train+1:N_train+N_v);
test_ = x(N_train+N_v+1:686);
PRECT = length(feature_filter_MulGeneP_AD_LMCI); %特征的总数 
